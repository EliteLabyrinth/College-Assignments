...
void sortedInsert(int *, int, int, int);
...
void enqueue(int *queue, int *r, int *c, int data){
    ...
    sortedInsert(queue, *r, *c, data);
    *r = ++(*r) % SIZE; ++(*c);
    printf("\n-- %d ENQUEUED\n", data);
}

void sortedInsert(int *queue, int r, int c, int data){
    if(c == 0 || data <= queue[r])	queue[(r + 1) % SIZE] = data;
    else{
        queue[(r + 1) % SIZE] = queue[r];
        sortedInsert(queue, (r + SIZE - 1) % SIZE, c - 1, data);
    }
}
...
void display(int *queue, int f, int c){
    if(c == 0){     printf("\n-- Empty Queue"); return; }
    printf("\nQueue:\t|F| ");
    while(c--)	printf("  %d  ", queue[f] * (1 + (f = (++f % SIZE)) / (f + 1)));
    printf(" |R|");
}
